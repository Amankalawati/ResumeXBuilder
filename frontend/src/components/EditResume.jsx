...
import { AlertCircle, ArrowLeft, Check, Download, Loader, Loader2, Palette, SaveIcon, Trash2 } from 'lucide-react'
...
import Modal from "../components/Modal";//you got error here so maybe change in future
...
//RESIZE OBSERVER HOOK( it will change the hook according to the page size)
...
const EditResume = () => {
  ...
  //Takes back to previous form state or take back to first or to /Dashbord
  const goBack = () => {
    const pages = [
      "profile-info",
      "contact-info",
...
//Update the array itme state using index
  ...
  //Adds the new item IE informtion
...
//Remove the info using the index
  ...
  //Fetching the resume detail using the backend url
 const fetchResumeDetailsById = async () => {
  ...
  //It will help in choosing the preview as well as helps in downloading the resume and also saves the resume as image
  const uploadResumeImages = async () => {
    try {
      ...
      //Stores images as Resume(in png format)
  ...
  // DELETE FUNCTOIN  FOR DELETING ANY RESUME CREATED
...
// DOWNLOAD FUNCTION.(it will help in downlaoding the pdf)
  ...
  //Type of how it will look when downloaded.
    try {
      await html2pdf()
        ...
        .from(element)
        .save(); // we save the resume here and can be downloded later when we need it ..
...
// all these fuction will help in editng the resumes
    ...
    <DashboardLayout>
        ...
        <Modal isOpen={openThemeSelector} onClose={() => setOpenThemeSelector(false)}
          ...
          <Modal isOpen={openPreviewModal} onClose={() => setOpenPreviewModal(false)}
            ...
            showActionBtn
            actionBtnText={isDownloading ? "Generatig...." 
